...
2. Create a new file `app/api/stringtale/route.ts` (or `src/app/api/stringtale/route.ts`) and add the following content:

    ```ts
      import { setupNextAppRoute } from "@stringtale/next"

      const { GET, POST } = setupNextAppRoute({
