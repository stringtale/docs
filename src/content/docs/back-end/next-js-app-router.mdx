...
2. Create a new file `app/api/stringtale.ts` and add the following content:
    ...
    ```tsx
      ...
      export default setupRoute({
        apiKey: process.env.STRINGTALE_TOKEN!,
      })
    ```
3. Add `STRINGTALE_TOKEN` to your `.env.local` file. This token can be obtained from the [Stringtale Dashboard](https://stringtale.com/app).
    ```
      STRINGTALE_TOKEN=YOUR_TOKEN
    ```
