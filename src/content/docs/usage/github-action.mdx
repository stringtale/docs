...
The GitHub Action automates the process of implementing text changes that have been made by you or your client. Your team will get notified automatically
...
You can either use the various triggers that GitHub has to offer (for example when pushing to a branch), or by letting
...
1. Navigate to the [Stringtale Dashboard](https://stringtale.com/app) and [obtain an API key](/usage/obtaining-api-key/).
...
4. Create a `stringtale.yaml` file in the `.github/workflows` folder in the root of your project. For example
...
Stringtale can automatically trigger the GitHub Action, using webhooks, whenever changes get saved. This does have the limitation that it only works
for the main branch. This is a limitation of GitHub Actions. If you want to run Stringtale on other branches, use the [push trigger](#pushing-to-a-branch) instead.
...
3. Navigate to the [Stringtale Dashboard](https://stringtale.com/app).
...
7. For type select 'GitHub', give it a name and for the URL enter `https://api.github.com/repos/{owner}/{repository}/dispatches`, replacing `{owner}` with the name of the owner of the repository (typically your organization) and `{repository}` with the name of the repository
   as they'd normally appear in the URL when you navigate to the repository on GitHub. For `Token` enter the fine-grained access token generated earlier.
