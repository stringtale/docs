...
## How do I use it?
 ...
 There are two ways to trigger the GitHub action. You can either use the various triggers that Github has to offer (we recommend pushes to a branch), or by letting 
...
1. Navigate to the [Stringtale Dashboard](https://stringtale.com/app) and [obtain yourself a Stingtale token](/usage/obtaining-token).
3. Go to your GitHub repository => Settings => Environments.
4. Add the `Deploy token` as `STRINGTALE_TOKEN` to the required environment(s) or create a new one.
...
1. Follow the steps above to set up the GitHub action, but replace the section `on: push:` with `on: repository_dispatch:` as follows:
   ```yaml
   name: Stringtale
...
6. Now whenever you press 'Save changes' in Stringtale, a webhook will automatically be fired to trigger the GitHub action.