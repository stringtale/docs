...
## Automatic wrapping of string iterpolated variables

The Visual Studio Code extension isn't limited to just static text. It supports automatically wrapping variables and placing them in
the format attribute of the `<Value>` tag.

So when you select the text and execute the `Stringtale: Wrap` command, the VS Code extension will automatically and populate the format attribute.
```tsx
