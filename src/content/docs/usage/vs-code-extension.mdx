...
3. Hit `Ctrl/Cmd + Shift + P` and execute `Stringtale: Wrap`.
...
5. Once you've accepted the name, the VS Code extension will wrap your text in a `<Value>` tag, rendering the field editable in your browser.
...
The demonstration above shows basic usage of how to wrap plain text with a `<Value>` tag. But the functionality of Stringtale isn't limited to just plain text. It
...
<Aside type="note">
  The `{/* HTML */}` prefix is used as a hint for your IDE to handle formatting correctly. Even though it's not strictly HTML, it gets the job done.
</Aside>
